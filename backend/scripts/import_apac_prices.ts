...
async function importAPACPriceHistory() {
  console.log("Starting APAC data import from CSV...");

  // Read CSV file
  const csvPath = join(__dirname, "../db/csv_data/apac.csv");
  ...
  for (const csvRow of rows) {
    ...
    // Get all dates from CSV (starting from 2018-01-31, but we'll import all available)
    const dateColumns = Object.keys(csvRow)
      .filter((key) => /^\d{4}-\d{2}-\d{2}$/.test(key))
      .sort();
    ...
    for (const dateStr of dateColumns) {
      ...
      // Import all dates (not just from 2000)
      try {
        await db.exec`
