 ...
 *
 * TEMPLATE (create a new city pool):
 * - Option A: Deploy MantleEstateCityLPPool directly with constructor params.
 * - Option B: Add a small wrapper like MantleEstateLPPoolHongKong below (recommended),
 *   so deployment requires only _usdcToken and the city metadata stays fixed in code.
 */
contract MantleEstateCityLPPool is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    /// @notice City identifier (e.g., bytes32("HKG"))
     ...
     * @param _cityCode City code, e.g., bytes32("HKG")
     * @param _cityName City name, e.g., "Hong Kong"
...
/**
 * @title MantleEstateLPPoolHongKong
 * @notice Preconfigured LP pool for Hong Kong (HKG).
 * @dev Deploy one instance per city; for now you asked to start with Hong Kong.
 */
contract MantleEstateLPPoolHongKong is MantleEstateCityLPPool {
    constructor(address _usdcToken)
        MantleEstateCityLPPool(
            _usdcToken,
            bytes32("HKG"),
            "Hong Kong",
            "Mantle Estate LP - Hong Kong",
            "meLP-HKG"
        )
    {}
}
